...
def test_mcp_capabilities_advertise_expected_registries(monkeypatch):
    ...
    * ``tools/list``     — exactly fifteen entries: three read-only added
      ...
      (``mask_file``), and one schema-drift tool added by S21-4
      (``detect_drift``). Stories beyond S21 will extend this set.
    ...
    * ``prompts/list``   — exactly four entries: the three workflow
    ...
    # EF-S2 + EF-S4 + EF-S5 + S7-4 + #407 + S21-1 + S21-2 + S21-3 + S21-4 —
    # fifteen tools: three read-only, three action, three onboarding, one
    # ad-hoc compare, one reconcile, one db-compare, one bulk-reconcile, one
    # PII-masking, one schema-drift, no more, no less.
