...
def api_client(monkeypatch):
    """Return a TestClient wrapping the FastAPI app, with a dev API key configured.

    The runs router now requires a valid X-API-Key (issue #9 fix 9-C). The
    fixture configures ``API_KEYS`` and attaches the matching header so
    test bodies do not need to repeat the auth wiring.
    """
    monkeypatch.setenv("API_KEYS", "test-key:admin")
    from src.api.main import app
    tc = TestClient(app)
    tc.headers.update({"X-API-Key": "test-key"})
    return tc
