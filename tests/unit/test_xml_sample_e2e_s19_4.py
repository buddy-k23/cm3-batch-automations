"""End-to-end validation of the XML worked sample (S19-4, #396).

Proves the committed sample under
``templates/etl/xml_single_record_sample/`` genuinely validates through the
real ``run_validate_service`` path (FormatDetector -> XmlParser ->
EnhancedFileValidator + the rule engine), and that the seeded defects surface
on the expected records. Mirrors ``test_json_sample_e2e_s19_2.py``.
"""
...
def test_sample_files_present():
    for name in ("input.xml", "mapping.json", "rules.json", "expected_report.json"):
...
def test_sample_validates_end_to_end_with_seeded_defects():
    ...
    by_rule = {(e.get("rule_id"), e.get("field")) for e in res["errors"]}
    # customer/@id absent record 9 (nested_required); status 'FROZEN' record 8;
    # age 'NaN' record 10; empty transactions record 6 (xml_array_length).
...
def test_sample_matches_committed_expected_report():
    """Re-running matches the committed expected_report.json (stable fields)."""
    res = _run()
...
def test_no_spurious_array_count_schema_warning():
    ...
    schema_noise = [
        ...
        if "TXN_COUNT" in str(i.get("field", "")) and i.get("category") == "schema"
    ]
    assert not schema_noise, f"unexpected schema noise on the array field: {schema_noise}"
