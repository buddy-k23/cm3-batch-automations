...
class TestCrossTypeValidator:
    ...
    # ---- header_trailer_count: empty/header-only batch (ADR 0013) ----------

    def test_header_trailer_count_empty_batch_passes_when_opted_in(self):
        """ADR 0013 Option A: count==0 and 0 detail rows is valid when opted in."""
        groups = {
            "detail": [],
            "trailer": [{"REC_TYPE": "TRL", "COUNT": "0"}],
        }
        rule = CrossTypeRule(
            check="header_trailer_count",
            record_type="trailer",
    ...
    def test_header_trailer_count_empty_batch_fails_without_opt_in(self):
        """Default (allow_empty_batch=False) keeps the empty case... clean too.

        With 0 declared and 0 rows the counts already match (0 == 0), so even
        without the opt-in there is no violation. This pins that the flag does
        not *introduce* a regression for the zero/zero case.
        """
        groups = {
            "detail": [],
            "trailer": [{"REC_TYPE": "TRL", "COUNT": "0"}],
    ...
    def test_header_trailer_count_truncated_file_still_fails_when_opted_in(self):
        """ADR 0013: a truncated file (declared > 0, 0 rows) is NOT exempted."""
        groups = {
            "detail": [],
            "trailer": [{"REC_TYPE": "TRL", "COUNT": "5"}],
        }
    ...
    def test_header_trailer_count_nonempty_unchanged_when_opted_in(self):
        """The opt-in does not alter non-empty-batch behaviour.

        A non-zero mismatch still fails; a non-zero match still passes.
        """
        mismatch = {
        ...
        assert len(self.validator.validate(mismatch, [rule], [])) == 1
        assert self.validator.validate(match, [rule], []) == []
...
# ---------------------------------------------------------------------------
# Regression tests for ADR 0006 / issue #12 — record_types dict coercion
# ---------------------------------------------------------------------------
#
# Before the schema fix, ``MultiRecordConfig.record_types`` was annotated as
# a bare ``dict``. Pydantic v2 does not coerce values for bare ``dict``
# annotations, so YAML-loaded callers got raw ``dict`` values where the
# validator expected ``RecordTypeConfig`` Pydantic models — and
# ``MultiRecordValidator`` crashed on ``type_config.position`` attribute
# access. Every existing test in this file constructs ``MultiRecordConfig``
# with already-instantiated ``RecordTypeConfig`` objects, which is exactly
# why the bug never surfaced. These tests exercise the YAML → Pydantic path
# that real callers (the API service, the multi-record CLI, and the
# pipeline runner via ``run_multi_record_validate_service``) use.


class TestRecordTypesDictCoercion:
    """Regression tests for the YAML → ``MultiRecordConfig`` construction path."""

    def test_record_types_values_coerced_to_record_type_config_from_dict(self):
        """Dict values for ``record_types`` are coerced to ``RecordTypeConfig``.

        This is the path ``yaml.safe_load(...)`` produces: every value is a
        plain ``dict``, not an already-built model. The schema must coerce.
        """
        raw = {
            "discriminator": {"field": "REC_TYPE", "position": 1, "length": 3},
            "record_types": {
                "header": {"match": "HDR", "mapping": "h.json", "expect": "exactly_one"},
                "detail": {"match": "DTL", "mapping": "d.json", "rules": "d_rules.json"},
                "trailer": {"position": "last", "mapping": "t.json"},
            },
        }
        cfg = MultiRecordConfig(**raw)
        for key, value in cfg.record_types.items():
            assert isinstance(value, RecordTypeConfig), (
                f"record_types[{key!r}] was {type(value).__name__}, "
                f"expected RecordTypeConfig"
            )
        # Spot-check attribute access — the very access pattern that used to
        # crash with ``AttributeError: 'dict' object has no attribute ...``.
        assert cfg.record_types["header"].match == "HDR"
        assert cfg.record_types["header"].expect == "exactly_one"
        assert cfg.record_types["detail"].rules == "d_rules.json"
        assert cfg.record_types["trailer"].position == "last"

    def test_record_types_loaded_from_yaml_file_runs_through_validator(self):
        """YAML → ``MultiRecordConfig`` → ``MultiRecordValidator.validate()``.

        This reproduces the exact failure mode reported in
        ``docs/handover/SHAW_atoctran_smoke_findings.md`` (bug #1): build a
        config from a YAML-derived dict and run it through the validator. The
        validator does ``type_config.position`` / ``type_config.match``
        attribute access internally; with the bug present this raised
        ``AttributeError``. With the schema fix it should complete cleanly
        on an empty file (no rows → no per-record validation → no errors).
        """
        import yaml
...
"""
        ...
        cfg = MultiRecordConfig(**raw)
        # All values must be coerced to RecordTypeConfig before the
        # validator accesses .position / .match.
        for value in cfg.record_types.values():
            assert isinstance(value, RecordTypeConfig)

        file_path = _write_lines([])  # empty file is the simplest exercise
        result = MultiRecordValidator().validate(str(file_path), cfg)
        # The bug crashed before any result dict was returned. Reaching this
        # point at all is the regression check; the ``valid`` payload just
        # confirms the run completed normally.
        assert result["total_rows"] == 0
        assert result["valid"] is True

    def test_positional_record_type_from_dict_preserves_position_attribute(self):
        """``position: 'first'`` from a YAML-derived dict round-trips cleanly.

        ``MultiRecordValidator._dispatch_rows`` reads
        ``type_config.position`` to handle first/last positional matches.
        Without the schema fix this is a ``KeyError`` on a raw dict (because
        the dict has ``"position"`` not the attribute ``position``) or an
        ``AttributeError`` on attribute access — depending on the caller.
        """
        raw = {
            "discriminator": {"field": "REC_TYPE", "position": 1, "length": 3},
            "record_types": {
                "first_row": {"position": "first", "mapping": ""},
                "rest": {"match": "DTL", "mapping": ""},
            },
        }
        cfg = MultiRecordConfig(**raw)
        assert isinstance(cfg.record_types["first_row"], RecordTypeConfig)
        assert cfg.record_types["first_row"].position == "first"
        assert cfg.record_types["first_row"].match == ""
