...
# Force-set API_KEYS so the value is deterministic across test ordering.
# ``setdefault`` is a no-op if another test module has already populated
# the var, which would leave us authenticating with the wrong credentials.
os.environ["API_KEYS"] = "test-key:mapping_owner"
...
def test_download_rules_not_found():
    ...
    r = client.get(
        "/api/v1/rules/definitely_missing_rules.json", headers=AUTH_HEADERS
    )
...
def test_download_rules_success(tmp_path, monkeypatch):
    ...
    r = client.get("/api/v1/rules/dl_test_rules.json", headers=AUTH_HEADERS)
    assert r.status_code == 200
