...
class TestScheduleApiEndpoints:
    ...
    The schedule router now requires a valid X-API-Key (issue #9 fix 9-C).
    Each test below ships ``X-API-Key`` and configures a dev key in env.
    """
    ...
    def test_get_schedules_returns_200(self, monkeypatch):
        ...
        with patch("src.services.scheduler_service.list_suites", return_value=[]):
            ...
            resp = client.get("/api/v1/schedules", headers=self._AUTH)
        assert resp.status_code == 200
        assert isinstance(resp.json(), list)

    def test_get_schedules_returns_suite_list(self, monkeypatch):
    ...
    def test_post_schedules_run_returns_202(self, monkeypatch):
        ...
        with patch("src.services.scheduler_service.run_suite_by_name", return_value=mock_result):
            client = TestClient(app)
            resp = client.post(
                "/api/v1/schedules/run",
                json={"suite_name": "test-suite"},
                headers=self._AUTH,
            )
        assert resp.status_code == 202

    def test_post_schedules_run_returns_run_id(self, monkeypatch):
    ...
    def test_post_schedules_run_suite_not_found_returns_error(self, monkeypatch):
        ...
        with patch("src.services.scheduler_service.run_suite_by_name", return_value=mock_result):
            ...
            resp = client.post(
                "/api/v1/schedules/run",
                json={"suite_name": "ghost-suite"},
                headers=self._AUTH,
            )
        # Accepts either 202 with error status in body, or 404
        assert resp.status_code in (202, 404)

    def test_post_schedules_run_missing_body_returns_422(self, monkeypatch):
        ...
        self._setup_keys(monkeypatch)
        client = TestClient(app)
        resp = client.post("/api/v1/schedules/run", json={}, headers=self._AUTH)
