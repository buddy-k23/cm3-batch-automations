...
# DB-to-file reconciliation template (S7-1, #375)
DB_RECON_TEMPLATE = TEMPLATES_DIR / "db_to_file_reconciliation.yml"
DB_RECON_SAMPLE_DIR = TEMPLATES_DIR / "db_to_file_reconciliation_sample"
DB_RECON_SAMPLE_EXTRACT_SQLITE = DB_RECON_SAMPLE_DIR / "extract.sqlite.sql"
DB_RECON_SAMPLE_EXTRACT_ORACLE = DB_RECON_SAMPLE_DIR / "extract.oracle.sql"
DB_RECON_SAMPLE_MAPPING = DB_RECON_SAMPLE_DIR / "mapping.json"
DB_RECON_SAMPLE_OUTPUT = DB_RECON_SAMPLE_DIR / "output.txt"
DB_RECON_SAMPLE_EXPECTED = DB_RECON_SAMPLE_DIR / "expected_report.json"
...
# ---------------------------------------------------------------------------
# S7-1 (#375) -- DB-to-file reconciliation template + sample
# ---------------------------------------------------------------------------


def test_db_recon_template_validates() -> None:
    """``db_to_file_reconciliation.yml`` loads cleanly through ``SourceConfig``.

    Mirrors the CSV / fixed-width template contracts: the committed YAML
    carries ``<FILL_IN_*>`` placeholders as string literals so the
    template validates as-shipped, and BAs see schema errors immediately
    if they misspell a key while replacing the placeholders.

    Asserts the required identity fields are preserved, the single
    declared ACTUAL output-file entry lands in ``cfg.input_files``, and
    the reconciliation-specific blocks (``expected_sql``,
    ``reconciliation``, ``tolerance``) survive as extras on the model
    (SourceConfig sets ``extra="allow"``).
    """
    assert DB_RECON_TEMPLATE.exists(), (
        f"DB-to-file reconciliation template missing at {DB_RECON_TEMPLATE}"
    )

    with DB_RECON_TEMPLATE.open("r", encoding="utf-8") as fh:
        raw = yaml.safe_load(fh)
    ...
    # One declared output-file entry with the documented placeholder values.
    assert len(cfg.input_files) == 1
    entry = cfg.input_files[0]
    assert entry.file_type == "ACTUAL"
    assert entry.glob == "<FILL_IN_OUTPUT_GLOB>"
    assert entry.mapping == "<FILL_IN_PATH_TO_MAPPING_JSON>"
    assert entry.target_staging_table == ""

    # Reconciliation-specific blocks preserved as extras for the CLI driver.
    extras = cfg.model_dump()
    assert "expected_sql" in extras
    assert extras["expected_sql"]["connection_env"] == "ORACLE"
    assert "reconciliation" in extras
    assert "key_columns" in extras["reconciliation"]
    assert extras["reconciliation"]["case_sensitive"] is True
    assert extras["reconciliation"]["trim_whitespace"] is True
    assert "tolerance" in extras


def test_db_recon_sample_zero_violations() -> None:
    """The DB-to-file sample reconciles cleanly against a SQLite extract.

    Acceptance criterion #3 from S7-1 (#375): the committed sample,
    invoked as ``valdo db-compare`` would invoke the comparator,
    returns **zero violations**. We exercise the SQLite path (no
    Docker / Oracle required in CI) by:

    * seeding an in-memory SQLite database with the same five
      ``SAMPLE_CUSTOMER`` rows the sample driver writes,
    * running ``extract.sqlite.sql`` to obtain the expected-rows
      DataFrame,
    * loading ``output.txt`` (pipe-delimited) into a DataFrame,
    * driving :class:`FileComparator` over the two sides on the
      ``CUSTOMER_ID`` key.

    The committed ``expected_report.json`` is regenerated by
    ``build_sample.py`` and pins the same contract: zero only-in-left,
    zero only-in-right, zero field-level differences, five matching
    rows.
    """
    assert DB_RECON_SAMPLE_EXTRACT_SQLITE.exists(), (
        f"Sample SQLite extract missing at {DB_RECON_SAMPLE_EXTRACT_SQLITE}"
    )
    assert DB_RECON_SAMPLE_EXTRACT_ORACLE.exists(), (
        f"Sample Oracle extract missing at {DB_RECON_SAMPLE_EXTRACT_ORACLE}"
    )
    assert DB_RECON_SAMPLE_MAPPING.exists(), (
        f"Sample mapping missing at {DB_RECON_SAMPLE_MAPPING}"
    )
    assert DB_RECON_SAMPLE_OUTPUT.exists(), (
        f"Sample output file missing at {DB_RECON_SAMPLE_OUTPUT}"
    )
    assert DB_RECON_SAMPLE_EXPECTED.exists(), (
        f"Sample expected report missing at {DB_RECON_SAMPLE_EXPECTED}"
    )

    # Seed an in-memory SQLite DB with the documented five rows.
    import sqlite3
    ...
    try:
        ...
        cur.execute(
            ...
            "EMAIL TEXT, BALANCE TEXT NOT NULL)"
        )
        cur.executemany(
            "INSERT INTO SAMPLE_CUSTOMER VALUES (?, ?, ?, ?)", sample_rows
        )
        conn.commit()
        sql = DB_RECON_SAMPLE_EXTRACT_SQLITE.read_text(encoding="utf-8")
        db_df = pd.read_sql_query(sql, conn, dtype=str)
    finally:
        conn.close()

    file_df = pd.read_csv(
        DB_RECON_SAMPLE_OUTPUT, sep="|", dtype=str, keep_default_na=False
    )
    ...
    # AC #3: zero violations across all three diff categories.
    only_left = result["only_in_file1"]
    only_right = result["only_in_file2"]
    only_left_records = (
    ...
    assert len(only_left_records) == 0, (
        f"Expected zero DB-only rows, got {only_left_records}"
    )
    assert len(only_right_records) == 0, (
        f"Expected zero file-only rows, got {only_right_records}"
    )
    assert result["rows_with_differences"] == 0, (
        f"Expected zero field-level diffs, got {result['differences']}"
    )
    assert result["matching_rows"] == 5
    assert result["total_rows_file1"] == 5
    assert result["total_rows_file2"] == 5

    # Cross-check the committed expected_report.json -- the build_sample.py
    # driver regenerates it from the same code path, so the contract is
    # pinned at commit time.
    with DB_RECON_SAMPLE_EXPECTED.open("r", encoding="utf-8") as fh:
        expected = json.load(fh)
    assert expected["workflow"]["status"] == "passed"
    assert expected["workflow"]["db_rows_extracted"] == 5
    assert expected["compare"]["rows_with_differences"] == 0
    assert expected["compare"]["matching_rows"] == 5
    assert expected["compare"]["only_in_file1"] == []
    assert expected["compare"]["only_in_file2"] == []
