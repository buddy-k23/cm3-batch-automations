...
# The webhook router now requires a valid X-API-Key (issue #9 fix 9-C). Every
# request below ships this header; tests configure ``API_KEYS`` via the
# ``_api_keys`` autouse fixture so the auth dependency permits the request.
_AUTH_HEADERS = {"X-API-Key": "dev-key"}


@pytest.fixture(autouse=True)
def _api_keys(monkeypatch):
    """Configure a dev API key so the webhook router auth dep accepts requests."""
...
class TestWebhookValidateEndpoint:
    ...
    def test_submit_returns_202_with_job_id(self):
        ...
        with patch(
            ...
            resp = client.post(
                "/api/v1/webhook/validate", json=payload, headers=_AUTH_HEADERS
            )
...
class TestWebhookJobStatus:
    ...
    def test_unknown_job_returns_404(self):
        ...
        resp = client.get(
            f"/api/v1/webhook/jobs/{fake_id}", headers=_AUTH_HEADERS
        )
    ...
    def test_known_job_returns_status(self):
        ...
        status_resp = client.get(
            f"/api/v1/webhook/jobs/{job_id}", headers=_AUTH_HEADERS
        )
