...
def _basename(value: str) -> str:
    ...
    The committed rules JSONs were originally generated on a Windows
    host so their ``metadata.template_path`` field carries a
...
def _normalise_path_metadata(data: dict) -> dict:
    """Coerce ``metadata.template_path`` to its basename.

    Used by the EC-S10 byte-equality regression test against the
    committed rules JSONs to neutralise the Windows-vs-POSIX path
    divergence that lives in the committed files (separate concern
    from EC-S10's timestamp work).
    """
    result = {**data}
    metadata = result.get("metadata")
    if isinstance(metadata, dict) and "template_path" in metadata:
        metadata = {**metadata}
        metadata["template_path"] = _basename(metadata["template_path"])
...
def test_emitted_flat_rules_json_matches_committed_for_known_good_file():
    ...
    divergence is ``metadata.template_path`` where the committed file
    carries a Windows-style absolute path
    ...
    artefacts = emit_rules_artefacts(workbook, frozen_timestamp=committed_ts)
    ...
    assert _normalise_path_metadata(emitted_data) == _normalise_path_metadata(
        ...
        "(after normalising template_path to basename; timestamps are "
        "synchronised via frozen_timestamp -- EC-S10)."
    )


def test_rules_emitter_is_byte_stable_with_frozen_timestamp():
    ...
    every rules JSON's ``metadata`` block) is closed; running
    ``valdo onboard-source --frozen-timestamp <value>`` twice now
    leaves the rules artefact tree's mtimes unchanged byte-for-byte.
    """
    import hashlib

    workbook = read_workbook(SHAW_WORKBOOK)

    artefacts_a = emit_rules_artefacts(workbook, frozen_timestamp="GENERATED")
    artefacts_b = emit_rules_artefacts(workbook, frozen_timestamp="GENERATED")

    assert len(artefacts_a) == len(artefacts_b), (
        f"Different rules artefact counts between runs: "
