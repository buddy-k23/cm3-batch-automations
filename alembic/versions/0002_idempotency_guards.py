"""Idempotency guards — handle pre-existing APP_RUN_HISTORY and APP_RUN_TESTS.

This migration adds columns that were introduced after the original manual DDL
at ``sql/app_int/setup_app_run_history.sql``.  Production instances that ran
...
- ``APP_RUN_HISTORY.quality_score``   — overall quality percentage for the run
- ``APP_RUN_HISTORY.run_duration_seconds`` — wall-clock seconds for the run
...
def upgrade() -> None:
    """Add new columns to APP_RUN_HISTORY for installs missing them.
    ...
    if _table_exists("APP_RUN_HISTORY") and not _column_exists(
        "APP_RUN_HISTORY", "quality_score"
    ):
        op.add_column(
            "APP_RUN_HISTORY",
    ...
    # Present in migration 0001 but absent from the original manual DDL.
    if _table_exists("APP_RUN_HISTORY") and not _column_exists(
        "APP_RUN_HISTORY", "run_duration_seconds"
    ):
        op.add_column(
            "APP_RUN_HISTORY",
