...
- feat(mcp): mask_file tool — PII masking (6 strategies) over MCP (S21-3, #435)
...
- feat(db): db-compare honors DB_ADAPTER via the factory (no OracleConnection fallback); runs on SQLite/Postgres/Oracle (S15-2, #405)
...
### Added
- feat(templates): XML single-record ETL template + worked sample + README; MCP auto-discovery (S19-4, #396)
- feat(parsers): XML parser (hardened lxml — XXE/billion-laughs defended) + XML XPath template column + xml_array_length validator, per ADR 0019 (S19-3, #396)
- feat(templates): JSON single-record ETL template + worked sample + README; wired JsonParser into the validate path so NDJSON validates end-to-end; MCP auto-discovery (S19-2, #395)
