"""Native (pandas + SQLite) comparison backend (S25-1).

This is the **default** :class:`~src.comparators.backends.base.ComparisonBackend`
implementation.  It wraps the engine routing and dispatch logic that
historically lived inline in
:func:`~src.services.compare_service.run_compare_service`, with **zero
behaviour change**: it routes to the in-memory
:class:`~src.comparators.file_comparator.FileComparator` or the set-based
:class:`~src.comparators.chunked_comparator.ChunkedFileComparator` exactly as
the service did before the seam was introduced.

The decision logic — ``use_chunked`` semantics, the ``.txt`` / ``.dat`` →
``'|'`` delimiter rule, fixed-width parser selection, the structure
compatibility check, and the key-column header / mapping fallback — is
preserved verbatim from the original service.
"""
...
def _build_fixed_width_specs(cfg: dict[str, Any]) -> list[tuple[str, int, int]]:
    """Build (name, start, end) byte-offset tuples from a fixed-width mapping.
...
class NativeComparisonBackend(ComparisonBackend):
    """Default backend wrapping the original FileComparator / ChunkedFileComparator dispatch.

    Routes to the set-based :class:`ChunkedFileComparator` when
    ``use_chunked`` is True, otherwise to the in-memory
    :class:`FileComparator`.  The result dicts are byte-for-byte identical to
    what ``run_compare_service`` produced before the seam was introduced.
    """
    ...
    def compare(
        ...
        *,
        ...
        """Compare two files using the native pandas / SQLite engines.

        See :meth:`src.comparators.backends.base.ComparisonBackend.compare`
        for the full contract.  This implementation preserves the exact
        routing and result shapes of the pre-seam ``run_compare_service``.
        """
        if use_chunked:
            if not key_columns:
                raise ValueError(
                    "Row-by-row comparison is not supported with chunked processing; provide keys."
                )
