...
class BARulesTemplateConverter:
    ...
    def from_csv(self, csv_path: str) -> Dict[str, Any]:
        ...
        """
        # Per ADR 0007: ``dtype=str`` preserves cell literals so a
        # purely-numeric ``Expected / Values`` column (e.g. an
        # all-``exact_length`` rules CSV with only integer thresholds)
        # does not get auto-inferred as ``float64`` and silently turn
        # ``100030`` into ``'100030.0'``. The SHAW ATOCTRAN rules CSVs
        # happened to escape this by accident (their column is mixed
        # with non-numeric tokens like ``CCYYMMDD``), but a tighter BA
        # spec would surface the bug. Closed defensively.
    ...
    def from_excel(self, excel_path: str, sheet_name: str | None = None) -> Dict[str, Any]:
        ...
        Raises:
            ValueError: If required columns are missing or a row contains an
                unsupported rule type.
        """
        # Per ADR 0007: ``dtype=str`` preserves cell literals — see
        # the docstring block in ``from_csv`` for the full rationale.
