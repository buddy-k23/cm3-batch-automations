"""Cross-record-type validation engine â€” issues #161/#162.
...
class CrossTypeValidator:
    ...
    def validate(
        ...
        Args:
            ...
            rules: List of :class:`~src.config.multi_record_config.CrossTypeRule`
                to evaluate.
            all_rows_types: Ordered list of record type names, one per
                non-empty row. ``None`` entries indicate rows whose record
                type could not be identified (unknown discriminator value).
    ...
    def _check_header_trailer_count(
        ...
        for row in type_rows:
            ...
            try:
                ...
                continue
            # ADR 0013 (Option A): an empty batch (0 declared, 0 counted) is
            # valid when the rule opts in via allow_empty_batch. Truncation
            # (declared > 0 with 0 rows) is NOT exempted and still fails below.
            if rule.allow_empty_batch and declared_int == 0 and actual_count == 0:
                continue
    ...
    def _check_type_sequence(
        ...
        Args:
            ...
            all_rows_types: Ordered list of record type names (one per
                non-empty row); ``None`` entries (unknown record types) are
                skipped when computing the observed sequence.
