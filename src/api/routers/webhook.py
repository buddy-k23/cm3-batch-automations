...
def _resolve_callback_allowlist(request: Request) -> Optional[list]:
    """Return the configured callback URL allowlist, or ``None`` if absent.

    The allowlist is read from ``security.callback_allowlist`` in
    ``config/ui.yml`` (loaded into ``app.state.ui_config`` at startup).
    When no allowlist is configured the validator falls back to
    DNS-resolution + private-range blocking only.
    ...
    Returns:
        A list of allowed hostnames, or ``None`` when not configured.
    """
    ui_cfg = getattr(request.app.state, "ui_config", {}) or {}
    security_cfg = ui_cfg.get("security", {}) or {}
    allowlist = security_cfg.get("callback_allowlist")
    if not allowlist:
        return None
    return [str(h) for h in allowlist]
...
async def submit_validation(
    ...
    Args:
        ...
        http_request: The underlying FastAPI ``Request`` object \u2014 used to
            access app config (callback allowlist).

    Returns:
        WebhookValidateResponse with the new job_id.

    Raises:
        HTTPException: 400 if the file_path does not exist on disk, or if
            ``callback_url`` fails SSRF safety validation.
    """
    ...
    # SECURITY: validate callback_url to prevent SSRF (cloud metadata,
    # internal network scanning, localhost pivots).
    if request.callback_url:
        try:
            assert_safe_callback_url(
                request.callback_url,
                allowlist=_resolve_callback_allowlist(http_request),
            )
        except ValueError as exc:
            raise HTTPException(
                status_code=400,
                detail=f"Invalid callback_url: {exc}",
            )
