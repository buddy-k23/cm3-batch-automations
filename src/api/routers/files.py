...
try:
    ...
    write_run_to_db = None  # type: ignore[assignment]
...
def _safe_upload_path(filename: str | None, *, prefix: str = "") -> Path:
    """Construct a sanitised path inside ``UPLOADS_DIR`` for an uploaded file.

    SECURITY: Multipart-uploaded ``file.filename`` is attacker-controlled
    and may contain path-traversal sequences (``../``), absolute paths,
    backslashes (Windows), or NUL bytes. Constructing
    ``UPLOADS_DIR / file.filename`` directly therefore allows writes
    outside the uploads directory \u2014 a CWE-22 path traversal.

    This helper:
      * Strips directory components via ``Path(filename).name``.
      * Rejects empty, ``.``, and ``..`` basenames.
      * Resolves the candidate path and verifies via ``is_relative_to``
        that it is contained within the resolved uploads directory.

    Args:
        filename: User-supplied filename from a multipart upload.
        prefix: Optional internal prefix to prepend to the safe basename
            (e.g. ``"validate_"``).

    Returns:
        A :class:`pathlib.Path` guaranteed to live inside ``UPLOADS_DIR``.

    Raises:
        HTTPException: 400 if the filename is missing, empty, or escapes
    ...
    # NUL byte protection \u2014 some filesystems silently truncate at NUL.
    if "\x00" in base:
        raise HTTPException(status_code=400, detail="Invalid filename")
    uploads_root = UPLOADS_DIR.resolve()
    candidate = (uploads_root / f"{prefix}{base}").resolve()
    try:
...
_LOGGER = get_structured_logger("valdo.files")
...
async def validate_file(
    ...
    upload_path = _safe_upload_path(file.filename, prefix="validate_")
    with open(upload_path, "wb") as buffer:
        shutil.copyfileobj(file.file, buffer)
...
def _run_compare_with_mapping(
    ...
    compare_file_result = FileCompareResult(
    ...
    _log_quick_test_to_db(
        test_type="compare",
        file_name=f"{upload_path1.name} vs {upload_path2.name}",
        mapping_id=request.mapping_id,
        result={
            "match": threshold_result.get("passed", False),
            "total_rows": compare_result['total_rows_file1'],
            "mismatch_count": compare_file_result.differences,
        },
        report_url=report_url or download_url,
    )

    return compare_file_result
...
async def compare_files(
    ...
    """
    upload_path1 = _safe_upload_path(file1.filename, prefix="file1_")
    upload_path2 = _safe_upload_path(file2.filename, prefix="file2_")
...
async def compare_files_async(
    ...
    upload_path1 = _safe_upload_path(file1.filename, prefix=f"job_{job_id}_file1_")
    upload_path2 = _safe_upload_path(file2.filename, prefix=f"job_{job_id}_file2_")
...
async def export_errors(
    ...
    upload_path = _safe_upload_path(file.filename, prefix="export_errors_")
    error_output_path = _safe_upload_path(file.filename, prefix="errors_")
    ...
    try:
        ...
        return FileResponse(
            ...
            filename=f"errors_{Path(file.filename or '').name}",
...
async def detect_drift_endpoint(
    ...
    upload_path = _safe_upload_path(file.filename, prefix="drift_")
    with open(upload_path, "wb") as buffer:
        shutil.copyfileobj(file.file, buffer)
...
def _log_quick_test_to_db(
    ...
    """Log Quick Test run to database.

    Args:
        test_type: Type of test ("validate" or "compare")
        file_name: Name of the uploaded file
        mapping_id: Mapping ID used (if any)
        result: Test result dictionary
        report_url: URL to the generated report
    """
    if write_run_to_db is None:
        return
    ...
    try:
        ...
        if test_type == "validate":
            ...
            fail_count = 0 if status == "passed" else 1
        else:  # compare
        ...
        # Create test entry
        test_entry = {
            "name": f"{test_type}: {file_name}" + (f" ({mapping_id})" if mapping_id else ""),
            "type": test_type,
            "status": status,
            "rows_processed": result.get("total_rows"),
            "error_count": result.get("invalid_rows", 0) if test_type == "validate" else result.get("mismatch_count", 0),
            "duration_secs": None,
            "report_path": report_url,
        }

        # Write to database
        write_run_to_db(run_entry, run_id, [test_entry])

    except Exception as exc:
        # Log but don't fail the request
        import logging
        logging.getLogger(__name__).warning(f"Failed to log Quick Test to database: {exc}")
