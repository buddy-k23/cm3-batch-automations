...
OUT = ROOT / "generated" / "e2e_app_int"
SUMMARY = OUT / "summary.txt"
SOURCE_CSV = OUT / "source_query_output.csv"
FINAL_FILE = OUT / "final_output.txt"
VALIDATION_CSV = OUT / "validation_results.csv"

TABLE_ACCOUNTS = "APP_DEMO_ACCOUNTS"
TABLE_CONTACTS = "APP_DEMO_CONTACTS"
TABLE_ACCT_CONTACTS = "APP_DEMO_ACCOUNT_CONTACTS"
...
def setup_demo_data(cur) -> None:
    ...
    cur.execute(
        ...
        CREATE TABLE {TABLE_ACCT_CONTACTS} (
          ...
          REL_SEQ NUMBER(3),
          CONSTRAINT PK_APP_DEMO_AC PRIMARY KEY (ACCOUNT_ID, CONTACT_ID),
          CONSTRAINT FK_APP_DEMO_AC1 FOREIGN KEY (ACCOUNT_ID) REFERENCES {TABLE_ACCOUNTS}(ACCOUNT_ID),
          CONSTRAINT FK_APP_DEMO_AC2 FOREIGN KEY (CONTACT_ID) REFERENCES {TABLE_CONTACTS}(CONTACT_ID)
        )
        """
    )
...
def main() -> None:
    ...
    env_path = Path("/Users/buddy/.openclaw/workspace/valdo/.env")
    ...
    try:
        ...
        with SUMMARY.open("w", encoding="utf-8") as f:
            f.write("APP_INT Oracle E2E Summary\n")
