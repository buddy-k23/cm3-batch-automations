...
5. **APP_INT baseline** (`docs/trials/2026-02-25-app_int-baseline-report.md`, `generated/e2e_app_int/wave_artifacts/*`)
...
| APP_INT baseline | PASS with 400 validated rows | `summary-report.json` + `telemetry.json` both show validated=400, failed=0, warned=0; promotion PASS | ✅ consistent |
...
- **APP_INT baseline:** aligned (`validated=400`, `failed=0`, `warned=0` across summary + telemetry).
...
3. **Promotion evidence schema inconsistency (medium):**
   ...
   - APP_INT uses a different shape (`policyVersion/enabled/preGateStatus/decision/reasonCodes/metrics`).
...
**P1**
4. **Unify/standardize promotion-evidence schema** across Wave and APP_INT flows.
