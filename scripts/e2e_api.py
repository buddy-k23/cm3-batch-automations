...
Prerequisites: Server running at https://127.0.0.1:8000 (override via VALDO_BASE_URL).
"""
...
BASE = os.getenv("VALDO_BASE_URL", "https://127.0.0.1:8000")
# Self-signed certs (tls.strategy: self_signed) fail verification by default.
# Set VALDO_VERIFY_TLS=1 once the cert is trusted locally.
...
def test_root_and_static() -> None:
    ...
    r = httpx.get(BASE + "/", timeout=10, follow_redirects=True, verify=VERIFY_TLS)
    check("GET /", r.status_code == 200)

    r = httpx.get(BASE + "/ui", timeout=10, follow_redirects=True, verify=VERIFY_TLS)
    check("GET /ui", r.status_code == 200)

    r = httpx.get(BASE + "/docs", timeout=10, follow_redirects=True, verify=VERIFY_TLS)
...
def main() -> int:
    ...
    try:
        httpx.get(f"{BASE}/api/v1/system/health", timeout=5, verify=VERIFY_TLS)
    except Exception as exc:
        print(f"\nERROR: Cannot reach server at {BASE}: {exc}", file=sys.stderr)
        sys.exit(2)

    print(f"\nValdo — E2E API Tests  ({run_date})")
