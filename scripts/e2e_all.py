...
def _check_server() -> None:
    ...
    try:
        httpx.get(f"{BASE_URL}/api/v1/system/health", timeout=5, verify=VERIFY_TLS)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Valdo E2E full regression suite")
    ...
    print(f"\n{BOLD}Valdo — E2E Full Regression Suite  ({run_date}){RESET}")
