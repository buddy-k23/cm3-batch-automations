...
Start the Valdo API server and use HTTP requests for validation. This is useful for integration testing or when you need programmatic access to results.
...
Use the composite action provided in `.github/actions/app-validate/`. This wraps the CLI with input/output handling, threshold evaluation, and artifact uploading.
...
| Package version | `app-version` | `appVersion` | `APP_VERSION` |
...
Verify the Valdo API is healthy after deployment.
...
post-deploy:
  ...
  steps:
    - name: Health check
      run: |
        STATUS=$(curl -sf "${{ secrets.APP_API_URL }}/api/v1/health" | python3 -c "
        import json, sys
        data = json.load(sys.stdin)
        print(data.get('status', 'unknown'))
        ")
        if [ "$STATUS" != "ok" ]; then
          echo "::error::Valdo API health check failed: status=$STATUS"
          exit 1
        fi
        echo "Valdo API is healthy"

    - name: Validate test file via API
      run: |
        curl -sf -X POST "${{ secrets.APP_API_URL }}/api/v1/validate" \
...
```bash
mkdir -p app-reports
valdo validate -f data.txt -m mapping.json -o app-reports/report.json
```
