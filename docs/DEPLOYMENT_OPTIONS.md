...
# Deploy to server
scp dist/valdo.pex server:/opt/valdo/
scp -r config server:/opt/valdo/

# On server
chmod +x /opt/valdo/valdo.pex
./valdo.pex
```
...
# Install on server
sudo yum install -y valdo-0.1.0-1.el8.noarch.rpm
...
ProtectHome=true
ReadWritePaths=/opt/valdo/logs /opt/valdo/data /opt/valdo/uploads /opt/valdo/config/mappings
...
podman run -d \
  --name valdo \
  -v /opt/valdo/config:/app/config:ro \
  -v /opt/valdo/data:/app/data \
  -v /opt/valdo/logs:/app/logs \
  --env-file /opt/valdo/.env \
  valdo:latest
```
...
Group=valdo
WorkingDirectory=/opt/valdo
Environment="PATH=/opt/valdo/venv/bin:/usr/local/bin:/usr/bin:/bin"
Environment="ORACLE_HOME=/opt/oracle/instantclient_19_23"
Environment="LD_LIBRARY_PATH=/opt/oracle/instantclient_19_23"
EnvironmentFile=/opt/valdo/.env
...
NoNewPrivileges=true
PrivateTmp=true
ReadWritePaths=/opt/valdo/logs /opt/valdo/uploads /opt/valdo/config/mappings
...
# Change ExecStart to: /opt/valdo/valdo.pex
...
# Install RPM
sudo yum install -y dist/valdo-*.rpm

# Migrate config
sudo cp /opt/valdo/.env /etc/valdo/
...
### 1. Dedicated User
```bash
sudo useradd -r -s /bin/false -d /opt/valdo valdo
```

### 2. File Permissions
```bash
sudo chmod 600 /opt/valdo/.env
sudo chmod 755 /opt/valdo
sudo chmod -R 750 /opt/valdo/config
```
