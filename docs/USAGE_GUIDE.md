...
tests:
  ...
  - name: P327 Oracle vs file
    ...
    file: data/p327_${run_date}.dat
    mapping: P327_full_in_sheet_order_strict
    oracle_query: "SELECT * FROM APP_INT.SHAW_SRC_P327 WHERE BATCH_DATE = :run_date"
    oracle_params:
      run_date: "${run_date}"
    key_columns: [LN]
    thresholds:
...
> **UI access requires `X-API-Key`.** The previous Referer-based bypass that
> allowed the bundled web UI to call protected endpoints without an API key
> has been **removed**. Every browser request from the UI must now carry an
> `X-API-Key` header. The bundled **API Tester** tab attaches the key
> automatically once you paste it into the API Key field, so day-to-day UI
> usage is unaffected — but custom front-ends, bookmarklets, or curl
> commands that previously relied on `Referer:` matching will now receive
> HTTP `401 Unauthorized` and must be updated to send the header
> explicitly.
...
ORACLE_USER=APP_INT                       # Default: APP_INT
...
ORACLE_SCHEMA=APP_INT                     # Default: value of ORACLE_USER
...
| `ORACLE_SCHEMA` | value of `ORACLE_USER` | Schema prefix for SQL table references (e.g. `APP_INT.APP_RUN_HISTORY`) |
...
```bash
curl http://app-server:8000/api/v1/schedules
...
```bash
curl -X POST http://app-server:8000/api/v1/schedules/run \
  -H "Content-Type: application/json" \
...
- `ci/gitlab-app-validate.yml` — GitLab CI include template
- `ci/azure-app-validate.yml` — Azure DevOps pipeline task
