...
GRANT SELECT ON CUSTOMER_DATA TO app_int;
GRANT SELECT ON TRANSACTIONS TO app_int;

-- Grant access to system views (usually granted by default)
GRANT SELECT ON USER_TABLES TO app_int;
GRANT SELECT ON USER_TAB_COLUMNS TO app_int;
GRANT SELECT ON USER_SEGMENTS TO app_int;
```
...
GRANT CREATE TABLE TO app_int;

-- Or grant specific permissions on existing log table
GRANT INSERT, SELECT ON TRANSACTION_LOG TO app_int;
```
...
WHERE grantee = 'APP_INT';

-- Check system privileges
SELECT * FROM USER_SYS_PRIVS
WHERE username = 'APP_INT';
```
...
- [x] User account created (e.g., `app_int`)
