...
| Environment tag     | `DEV`               | `APP_ENVIRONMENT` env var     |
...
```ini
[monitor:///opt/valdo/logs/audit.jsonl]
sourcetype = _json
index = app_audit
disabled = false
```
...
```spl
index=app_audit event="test_run_completed" valid=false
...
```spl
index=app_audit event="auth_failure"
...
```spl
index=app_audit event="suite_step_completed"
...
```spl
index=app_audit event="test_run_completed"
...
```spl
index=app_audit event="test_run_started" file_hash=*
