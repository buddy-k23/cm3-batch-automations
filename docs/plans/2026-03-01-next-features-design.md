...
`valdo watch` command polls the directory, matches the trigger to a suite YAML,
...
**Manual override** — `valdo run-tests` already works for single reruns.
...
### Components
- `src/commands/watch_command.py` — `valdo watch --dir /batch/triggers --suites config/test_suites/`
- `src/api/routers/runs.py` — `POST /api/v1/runs/trigger`, `GET /api/v1/runs/{run_id}`
- `ci/gitlab-app-validate.yml`
- `ci/azure-app-validate.yml`
