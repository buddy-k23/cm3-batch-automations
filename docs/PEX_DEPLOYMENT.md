...
```bash
sudo mkdir -p /opt/valdo/{config,data/samples,data/mappings,logs}
...
```bash
# Copy PEX file
sudo cp dist/valdo.pex /opt/valdo/
sudo chmod +x /opt/valdo/valdo.pex

# Copy configuration files
sudo cp -r config/* /opt/valdo/config/
sudo cp .env.example /opt/valdo/.env
...
# Replace PEX file
sudo cp dist/valdo.pex /opt/valdo/
sudo chown valdo:valdo /opt/valdo/valdo.pex
sudo chmod +x /opt/valdo/valdo.pex
