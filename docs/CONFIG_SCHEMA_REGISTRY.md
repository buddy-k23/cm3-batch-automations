...
| **Source YAML** (per-source overlay) | `config/e2e/sources/SHAW.yml` | `scripts/e2e_lib/path_resolver.py` → `PathResolver.source_config` / `_compile_source_patterns` / `trigger_config`. **EA-S1** adds the Pydantic contract `src/pipeline/etl_config.py` → `SourceConfig` / `OutputFileConfig` / `InputFileConfig` / `ToleranceConfig` / `ThresholdsConfig` (schema-only; not yet wired into the runtime loader). | **Partial.** Validates: top-level is a mapping, `source` matches the filename, and that the overlay does **not** redefine env-forbidden keys (`_SOURCE_OVERRIDE_FORBIDDEN_KEYS`). **R-07 optional overrides** (validated when present): `filename_patterns` (a non-empty list compiled with the same contract as the global block — unique names, valid regex, `source`/`file_type` named groups, valid `direction`; **replaces** the global list for that source) and `trigger_file.{suffix,data_file_line}` (non-empty string suffix; integer `data_file_line` ≥ 1; each key falls back to the global default when absent). **Does NOT validate `schema_version`** (the file declares `schema_version: 1` but no loader checks it) and does not reject other unknown keys. **CI guardrail (EB-S2):** `tests/unit/test_source_yaml_guardrails.py` walks every `config/e2e/sources/*.yml` and rejects (a) legacy multi-record keys (`multi_record`, `discriminator_field`) at the `output_files[]` level (EB-S1 / ADR 0005) and (b) default-equal boilerplate per the EA-S1 Pydantic defaults (`strict_fixed_width: true`, `strict_level: all`, `tolerance.{max_errors,max_error_pct,ignore_fields}` at zero/empty, `thresholds.max_errors: 0`), preserving non-default override siblings (e.g. SRC_A's `tolerance.ignore_fields`). |
