...
```markdown
[![pipeline status](https://trgl.gitlab-dedicated.com/org/APPID-33091157-Developers/valdo/badges/main/pipeline.svg)](https://trgl.gitlab-dedicated.com/org/APPID-33091157-Developers/valdo/-/commits/main)

[![coverage report](https://trgl.gitlab-dedicated.com/org/APPID-33091157-Developers/valdo/badges/main/coverage.svg)](https://trgl.gitlab-dedicated.com/org/APPID-33091157-Developers/valdo/-/commits/main)
```
...
## E2E execution modes (one engine, selectable mode)

The E2E batch-testing harness runs in distinct **modes** that all delegate to the
same orchestrator (`scripts/e2e_lib/run_source.py`) — no mode forks a parallel
path (guarded by `tests/unit/test_mode_parity.py`). Use the mode-aware wrapper:
...
| Mode | What it is | Engine | Notes |
|---|---|---|---|
| `integration` | SIT/integration batch gating run | `run_source.py` | Default historical behaviour; blocking gates fail the run. |
| `uat` | UAT-style run | `run_source.py` | **No separate `uat` environment** (only `sit`/`ait` this iteration). UAT data/baselines are selected by per-source config + baseline pinning on an approved env; sign-off is downstream of the same exit code. |
| `ci` | CI/CD pipeline stage | `run_source.py` | Contract is the exit code (0 pass / 2 blocking-failure / 3 infra). Java shell-outs default OFF in CI (`VALDO_E2E_DISABLE_JAVA=1`; set `0` to re-enable). |

`--env` accepts only `sit` or `ait`; the wrapper rejects any other value so a
`uat`/`prod` environment is never silently invented (AGENTS.md). Exit codes are
identical across modes (mirrored from `run_source.py`).
