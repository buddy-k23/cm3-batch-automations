# Pipeline Regression Testing Guide (Java ETL + Valdo Validator)

This guide shows how to use Valdo as a regression framework around an existing multi-step Java batch process.

## 1) Target Architecture

## 0) app_int Sample Schema + Test Data Setup

Use the provided SQL to create regression sample tables and rows in `app_int`:

- `sql/app_int/setup_app_int_regression_samples.sql`

Example run (SQL*Plus):

```bash
sqlplus user/password@dsn @sql/app_int/setup_app_int_regression_samples.sql
```

This creates source-system sample tables:
- `app_int.src_a_accounts`, `app_int.src_a_balances`
- `app_int.src_b_accounts`, `app_int.src_b_balances`
...
- query files in `config/queries/app_int/`
...
Regression mapping/rules added for app_int samples:
- `config/mappings/p327_app_int_regression_mapping.json`
- `config/rules/p327_app_int_regression_rules.json`
