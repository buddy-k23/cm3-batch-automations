...
1. **Core engine gate** — configured in `pytest.ini` (`--cov-fail-under=80`
   ...
   ```bash
   # full suite with the configured coverage gate (src/ engine scope, 80% — ADR 0011)
   .venv/bin/pytest -q

   # targeted verification packs (coverage options disabled for a focused run)
   .venv/bin/pytest -q -o addopts='' \
     tests/unit/test_workflow_engine.py tests/unit/test_workflow_wrapper_parity.py
   ```
...
> **Note:** Running a subset of tests (e.g. a single file) under the configured
> `addopts` reports 0% coverage and fails the gate — use `-o addopts=''` for
> focused runs, or run the full `tests/unit/` suite for a valid coverage check.

## Workflow Verification Commands

Pre-merge verification workflows (config-driven; safe to run on a feature
branch):
...
Notes:
- Some manifest scenarios are intentionally invalid; these are expected
  failures and should be captured in telemetry.
- Strict fixed-width validation is available in both non-chunked and chunked
  validate paths.
